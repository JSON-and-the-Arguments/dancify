  ...
  return (
    <View className="w-60 h-80 pt-50  border-red-500/10 m-5 items-center">
      ...
      <View className=" flex-row mx-10 justify-between w-full h-30 items-center">
        <View >
          <Text className="text-white text-3xl mb-2  decoration-2">{user.firstname}</Text>
          <Text className="text-white text-xl capitalize">{user.dancestyles}</Text>
          <Text className="text-white text-xl">{user.role}</Text>
        </View>
        <TouchableOpacity
        onPress={() => navigation.navigate('Chat')}>
        <View className='ml-10'>
          <MaterialCommunityIcons user={user} name="message" size={46} color="white" />
        </View>
        </TouchableOpacity>
